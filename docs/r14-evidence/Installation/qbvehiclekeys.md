...
title: Updating qb-vehiclekeys
description: Adding support for the police slim jim. 
...
a single network event to our ```qb-vehiclekeys/client/main.lua``` anywhere in the resource so that the ```lastPickedVehicle``` variable will
...
```lua title='Custom qb-vehiclekeys Networked Event Handler' showLineNumbers
RegisterNetEvent('qb-vehiclekeys:client:UpdateLastPicked', function(entity)
    lastPickedVehicle = entity
end) 
```

If you use a third-party vehicle keys script, you will either need to reach out to the developer or attempt to examine the code yourself
to enable the functionality of the access tool. 

## Preconfigured Client File

:::caution

This code is current as of 10/31/2022, it will not contain updates more recent than this, and may not be compatible with older version of qb-vehiclekeys!

:::

```lua title='Modified qb-vehiclekeys/client/main.lua' showLineNumbers
-----------------------
----   Variables   ----
-----------------------
