...
title: Updating qb-policejob
description: Removing the qb-policejob/client/evidence.lua and duplicated server events.

---

Because r14-evidence uses many of the event names as qb-policejob to maintain compatability with existing
resources, we will need to remove the existing code which provides for evidence generation and handling 
from the ```qb-policejob/client/evidence.lua``` and ```qb-policejob/server/main.lua``` files. Additionally,
if you have resources which are clones of ```qb-policejob```, such as ```qb-fib```, you will need to repeat
this process for those as well! You do not need multiple versions of evidence per job, and can instead include
all jobs that need access to evidence under the included ```Config.AuthorizedJobs.LEO``` table.
...
## Deleting Existing Evidence Files

The first step of our process will be simply deleting the ```qb-policejob/client/evidence.lua``` file from our 
resource, this will prevent the duplication of evidence creation when firing a weapon, and prevent an unecessary
hit to performance caused by having an unused evidence system running in qb-policejob.

![Deleting evidence.lua from our qb-policejob resource](/img/qbpolicejob.png)

## Removing Existing Server Events

Next, we will need to access our ```qb-policejob/server/main.lua``` and remove the following events which are
...
in r14-evidence. Please use CTRL-F to search for, and entirely remove, these networked event handlers. 

:::info

If you have resources which are cloned versions of qb-policejob, you may need to search them for events with these names as well!
...
## Removing Existing Server Callbacks

We will need to remove the `police:GetPlayerStatus` callback as well from our ```qb-policejob/server/main.lua``` file.
Because we want this callback available for the Check Status radial menu option, it will need to be moved to r14-evidence
so that it can properly supply status effects to the client using it. 

```lua title='qb-policejob Server Callback To Be Removed' showLineNumbers 
...
## Removing Existing Server Commands

Lastly we will need to remove the ```'clearblood'``` and ```'clearcasings'``` commands from our qb-policejob server
script as these have been replaced by a combined ```'clearevidence'``` command in r14-evidence. Once we have done this,
we will finally need to remove the ```'takedna'``` command, as the new resource now allows police to use qb-target in
order to collect evidence versus a command.

```lua title='qb-policejob Server Command To Be Removed' showLineNumbers 
...
At this point we have completed the modification of our qb-policejob script and can now continue with our installation process!

## Preconfigured Server File

If you are use a recent version of qb-core and a stock qb-policejob, you may be able to use the modified version of ```qb-policejob/server/main.lua``` below that comes with
all of these these events plus evidence-related functions and variables removed. If you have custom code in your ```qb-policejob/server/main.lua```, please follow the process
bove for modifying your sever code!

:::caution

This code is current as of 11/3/2022, it will not contain updates more recent than this, and may not be compatible with older version of qb-policejob!

:::

```lua title='Modified qb-policejob/server/main.lua' showLineNumbers
-- Variables
local Plates = {}
local Objects = {}
